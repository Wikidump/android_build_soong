...
var (
	...
	lineageResTag    = dependencyTag{name: "org.etucqzs.platform-res"}
...
func decodeSdkDep(ctx android.BaseContext, v string) sdkDep {
	...
	case "":
		return sdkDep{
			...
			lineageResModule:   "org.etucqzs.platform-res",
		}
...
func (j *Module) deps(ctx android.BottomUpMutatorContext) {
	if ctx.Device() {
		...
		if ctx.ModuleName() == "org.etucqzs.platform-res" {
			ctx.AddDependency(ctx.Module(), frameworkResTag, "framework-res")
		}
		if ctx.ModuleName() == "org.lineageos.platform" ||
			...
			ctx.AddDependency(ctx.Module(), lineageResTag, "org.etucqzs.platform-res")
		}
	}
		...
		case Dependency:
			...
			case lineageResTag:
				if ctx.ModuleName() == "org.lineageos.platform" ||
					...
					// generated by org.etucqzs.platform-res.apk
					deps.srcJars = append(deps.srcJars, dep.(*AndroidApp).aaptSrcJar)
				}
